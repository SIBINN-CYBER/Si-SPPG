...
<div class="main">
    ...
    <form action="{{ route('menu') }}" method="GET" style="margin-bottom: 20px; text-align: center;">
        <input type="text" name="search" placeholder="Cari Nama Menu..." value="{{ $search }}" class="search-input">
        <button type="submit" class="search-button">Cari</button>
    </form>
    <div class="menu-container">
        @foreach($menus as $row)
            <div class="menu-card">
                <img src="{{ asset('assets/uploads/' . $row->foto) }}" alt="{{ $row->namamenu }}">
                <div class="menu-content">
                    <h3>{{ $row->namamenu }}</h3>
                    <p class="date">Tanggal: {{ date('d F Y', strtotime($row->tanggal)) }}</p>
                    <div class="menu-details">
                        <p><strong>Menu Utama:</strong> {{ $row->menu_utama }}</p>
                        <p><strong>Lauk:</strong> {{ $row->lauk }}</p>
                        <p><strong>Saus:</strong> {{ $row->saus }}</p>
                        <p><strong>Dessert:</strong> {{ $row->dessert }}</p>
                        <button class="btn-gizi" onclick='openGiziModal(@json($row), "besar")'>Lihat Gizi Besar</button>
                        <button class="btn-gizi" onclick='openGiziModal(@json($row), "kecil")'>Lihat Gizi Kecil</button>
                    </div>
                </div>
            </div>
        @endforeach
